...
void calc_conductance(WeightedGraph& wg, Graph& g, double& modularity, int& file_no)
{
	...
	// then the conductance (phi) is E / D
	...
	ofstream fout(file_name.c_str()); // file that stores conductance values
	int community_degree; // sum of the degrees of each node in a community
	int total_crossing_edges;

	std::unordered_map<pair<int, int>, int >::iterator cross_edges_it;
	pair<int, int> edge;
	std::vector<pair<int, double> > conductances;
	...
	for(int i = 0; i < wg.num_vertices; ++i)
	{
		...
		for(unsigned int j = 0; j < wg.vertex[i].neighbors.size(); ++j)
		{
			...
			cross_edges_it = wg.edges.cross_edges.find(edge);
...
void calc_conductance(WeightedGraph& wg, Graph& g)
{
	...
	ofstream fout("final_conductance"); // file that stores conductance values
	int community_degree; // sum of the degrees of each node in a community
	int total_crossing_edges;
...
//	calc_conductance(best_wg, g);
    ...
    for (int i = 0; i < g.num_vertices; i++)
    {
        ants[i].location.neighbors = nullptr;
        ants[i].location.common = nullptr;
    }
...
//		vector<int>::iterator inter_it = set_intersection(potential_clique.begin(),
//														  potential_clique.end(),
//														  g.vertex[*it].neighbors.begin(),
//														  g.vertex[*it].neighbors.end(),
//														  intersection.begin());
...
void calcNeighborhoodSize(Graph* graph)
{
	for(int i=0; i < graph->num_vertices; i++)
	{
		...
		//graph->vertex[i].common.reserve(graph->vertex[i].degree);
...
//			it = set_intersection(graph->vertex[i].neighbors.begin(),
//								  graph->vertex[i].neighbors.end(),
//								  graph->vertex[neighbor].neighbors.begin(),
//								  graph->vertex[neighbor].neighbors.end(),
//								  intersection.begin());
...
//            wg.displayGraph();
//
//            std::cout << "\n\nMoving node " << i+1 << " from " << curr_comm << " to " << best_comm << "\n";
//            std::cout << "Best gain = " << best_gain << "\n";
//            std::cout << "Indegree of node " << i+1 << " is " << c.in_degree[i] << "\n";
